...
<div class="container-laporan mb-5">
    <div class="header">
        <h4 class="fw-bold" style="color: #1e293b;">Daftar Produk</h4>
    ...
    <div class="offcanvas offcanvas-bottom offcanvas-filter" tabindex="-1" id="filterProduk" style="height:auto; max-height:80vh; border-radius:20px 20px 0 0;">
        ...
        <form method="GET" action="{{ route('item.index') }}" class="d-flex flex-column">
            <div class="offcanvas-body">
                <label>Cari Produk</label>
                <input type="text" name="search" class="form-control" placeholder="Ketik nama atau kode produk..." value="{{ request('search') }}">
                <label>Kategori</label>
                <div class="filter-chip-group">
                    <label class="filter-chip {{ request('kategori') ? '' : 'active' }}">
                        <input type="radio" name="kategori" value="" class="d-none" {{ request('kategori') ? '' : 'checked' }}> Semua
                    </label>
                    @foreach($kategoris as $kategori)
                        <label class="filter-chip {{ request('kategori') == $kategori->id ? 'active' : '' }}">
                            <input type="radio" name="kategori" value="{{ $kategori->id }}" class="d-none" {{ request('kategori') == $kategori->id ? 'checked' : '' }}>
                            {{ $kategori->kategori }}
                        </label>
                    @endforeach
                </div>
            ...
            <div class="offcanvas-footer">
                <a href="{{ route('item.index') }}" class="btn btn-outline-dark fw-bold flex-fill">Reset</a>
                    ...
                    <tr>
                        ...
                        <td class="td-action">
                            <div class="d-flex flex-wrap gap-2 justify-content-center">
                                <a href="{{ route('item.show', $item->kode_barang) }}" class="btn-action text-white shadow-sm" style="background-color: #3b82f6;" title="Lihat Detail Produk">
                                ...
                                <a href="{{ route('item.edit', $item->kode_barang) }}" class="btn-action text-dark shadow-sm" style="background-color: #facc15;" title="Edit Produk">
                                ...
                                <form id="delete-form-{{ $item->kode_barang }}" action="{{ route('item.destroy', $item->kode_barang) }}" method="POST" style="margin: 0;">
                                    @csrf @method('DELETE')
                                    <button type="button" class="btn-action text-white shadow-sm" style="background-color: #ef4444;" title="Hapus Produk" onclick="confirmDelete('{{ $item->kode_barang }}')">
            ...
            <div class="mobile-card-item">
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end action-dropdown-menu">
                        <li>
                            <a class="dropdown-item" href="{{ route('item.show', $item->kode_barang) }}">
                        ...
                        <li>
                            <a class="dropdown-item" href="{{ route('item.edit', $item->kode_barang) }}">
                        ...
                        <li>
                            <button type="button" class="dropdown-item text-danger" onclick="confirmDelete('{{ $item->kode_barang }}')">
                ...
                <form id="delete-form-{{ $item->kode_barang }}" action="{{ route('item.destroy', $item->kode_barang) }}" method="POST" class="d-none">
                    @csrf @method('DELETE')
                </form>
            </div>
        @empty
