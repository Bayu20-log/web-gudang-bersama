...
<div class="container-laporan">
    ...
    <form method="GET" class="filter-form">
        ...
        <div class="form-group">
            <a href="{{ route('laporan.stok.viewer') }}" class="back-button">Reset Filter</a>
        </div>
    </form>
    ...
    <table>
            <thead>
                @php
                    // pastikan variabel ada (fallback ke request)
                    $sortBy  = $sortBy  ?? request('sort_by', 'nama_barang');
                ...
                <tr>
                    @foreach($columns as $key => $label)
                        <th>
                            <a href="{{ route('laporan.stok.viewer', array_merge(request()->all(), [
                                'sort_by'  => $key,
                                'sort_dir' => ($sortBy === $key && strtolower($sortDir) === 'asc') ? 'desc' : 'asc'
                            ])) }}">
                                {{ $label }}
                                @if($sortBy === $key)
                                    {{ strtolower($sortDir) === 'asc' ? '↑' : '↓' }}
                                @else
                                    ▲▼
                                @endif
                            </a>
