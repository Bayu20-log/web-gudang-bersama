...
<div class="container-laporan mb-5">
    <div class="header">
        <h4 class="fw-bold" style="color: #1e293b;">Master Data &rsaquo; Daftar Satuan</h4>
        <a href="{{ route('satuan.create') }}" class="btn btn-orange fw-bold px-4 py-2 shadow-sm">+ Tambah Satuan</a>
    </div>
    ...
    <div class="search-filter-bar">
        <form method="GET" action="{{ route('satuan.index') }}" class="search-bar-form">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z"/></svg>
            <input type="text" name="search" placeholder="Cari nama satuan..." value="{{ request('search') }}" onchange="this.form.submit()">
        </form>
        @if(request('search'))
            <a href="{{ route('satuan.index') }}" class="filter-icon-btn" title="Reset pencarian">
        ...
        <table>
            ...
            <tbody>
                @forelse($satuans as $index => $satuan)
                    <tr>
                        <td class="fw-medium text-secondary">{{ $satuans->firstItem() + $index }}</td>
                        <td class="fw-bold" style="color: #1e293b;">{{ $satuan->nama_satuan }}</td>
                        <td>{{ optional($satuan->created_at)->format('d-m-Y H:i') }}</td>
                        <td>{{ optional($satuan->updated_at)->format('d-m-Y H:i') }}</td>
                        <td class="td-action">
                            <div class="d-flex flex-wrap gap-2 justify-content-center">
                                <a href="{{ route('satuan.edit', $satuan->id) }}" class="btn-action text-dark shadow-sm" style="background-color: #facc15;" title="Edit Satuan">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/><path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/></svg>
                                </a>
                                <form id="delete-form-{{ $satuan->id }}" action="{{ route('satuan.destroy', $satuan->id) }}" method="POST" style="margin: 0;">
                                    @csrf @method('DELETE')
                                    <button type="button" class="btn-action text-white shadow-sm" style="background-color: #ef4444;" title="Hapus Satuan" onclick="confirmDelete('{{ $satuan->id }}')">
                ...
                @empty
                    <tr><td colspan="5" class="text-center py-5 text-muted fw-medium"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="mb-2" style="color: #cbd5e1;" viewBox="0 0 16 16"><path d="M.54 3.87.5 3a2 2 0 0 1 2-2h3.672a2 2 0 0 1 1.414.586l.828.828A2 2 0 0 0 9.828 3h3.982a2 2 0 0 1 1.992 2.181l-.637 7A2 2 0 0 1 13.174 14H2.826a2 2 0 0 1-1.991-1.819l-.637-7a1.99 1.99 0 0 1 .342-1.31zM2.19 4a1 1 0 0 0-.996 1.09l.637 7a1 1 0 0 0 .995.91h10.348a1 1 0 0 0 .995-.91l.637-7A1 1 0 0 0 13.81 4H2.19zm4.69-1.707A1 1 0 0 0 6.172 2H2.5a1 1 0 0 0-1 .981l.006.139C1.72 3.042 1.95 3 2.19 3h5.396l-.707-.707z"/></svg><br>Data satuan belum tersedia.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<script>
    function confirmDelete(id) {
        Swal.fire({
            title: 'Hapus Satuan?',
            text: "Data satuan ini akan dihapus secara permanen!",
            icon: 'warning',
