...
<style>
        ...
        .section-title {
                ...
                display: flex;
                align-items: center;
                justify-content: space-between; /* Menambahkan properti ini untuk memisahkan teks & ikon info */
                border-bottom: 2px solid #f8fafc;
                padding-bottom: 10px;
        }
        
        /* Gaya khusus untuk ikon info (i) */
        .info-icon {
                color: #94a3b8;
                cursor: pointer;
...
<div class="app-container">
        ...
        <div id="tour-transaksi" class="category-wrapper">
                <div class="section-title">
                        <div><i class="bi bi-box-seam text-orange me-2 fs-5"></i> Transaksi & Produk</div>
                        <i class="bi bi-info-circle info-icon fs-5" onclick="showInfo('transaksi')" title="Info Kontainer"></i>
                </div>
        ...
        <div id="tour-master" class="category-wrapper">
                <div class="section-title">
                        <div><i class="bi bi-database-fill text-blue me-2 fs-5"></i> Master Data</div>
                        <i class="bi bi-info-circle info-icon fs-5" onclick="showInfo('master')" title="Info Kontainer"></i>
                </div>
                <div class="grid-container">
        ...
        <div id="tour-laporan" class="category-wrapper">
                <div class="section-title">
                        <div><i class="bi bi-file-earmark-bar-graph-fill text-teal me-2 fs-5"></i> Laporan</div>
                        <i class="bi bi-info-circle info-icon fs-5" onclick="showInfo('laporan')" title="Info Kontainer"></i>
                </div>
                <div class="grid-container">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Objek konfigurasi untuk Tour Utama
    const mainDriverObj = driver({
        showProgress: true,
        animate: true,
        doneBtnText: 'Selesai',
    ...
    // 1. Cek apakah ini kunjungan pertama (key localStorage belum ada)
    if (!localStorage.getItem('gudangkuTourSelesai')) {
        // Jika belum pernah, jalankan tour
        mainDriverObj.drive();
        // Set flag agar di kunjungan berikutnya tidak muncul lagi
        localStorage.setItem('gudangkuTourSelesai', 'true');
    }

    // 2. Fungsi terpisah untuk memunculkan informasi kontainer ketika ikon (i) di-klik
    window.showInfo = function(section) {
        let stepConfig = {};
        
        if (section === 'transaksi') {
            stepConfig = {
                element: '#tour-transaksi',
                popover: { title: 'Akses Cepat Transaksi', description: 'Semua menu untuk menambah dan mengecek barang masuk atau keluar terpusat di sini.', side: "top", align: 'start' }
            };
        } else if (section === 'master') {
            stepConfig = {
                element: '#tour-master',
                popover: { title: 'Kelola Master Data', description: 'Pusat pengaturan seluruh data inti, mulai dari daftar produk, kategori, pemasok, lokasi gudang, hingga satuan barang.', side: "top", align: 'start' }
            };
        ...
        // Jalankan instance driver khusus untuk satu langkah (single popover)
        const infoDriver = driver({
            showProgress: false,
            animate: true,
            doneBtnText: 'Paham',
            steps: [stepConfig]
