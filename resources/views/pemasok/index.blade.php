...
<div class="container-laporan mb-5">
    ...
    <div class="table-wrapper desktop-table-wrapper">
        <table>
            <thead>
                <tr>
                    <th>No</th><th>Nama Pemasok</th><th>Email</th><th>Jenis</th><th>Alamat</th>
                    <th>No. Telepon</th><th>Nama PIC</th><th>Bergabung Sejak</th><th>Diperbarui pada</th><th>Aksi</th>
                </tr>
            </thead>
            <tbody>
                    ...
                    <tr>
                        ...
                        <td class="td-action">
                            <div class="d-flex flex-wrap gap-2 justify-content-center">
                                <a href="{{ route('pemasok.edit', $pemasok->id) }}" class="btn-action text-dark shadow-sm" style="background-color: #facc15;" title="Edit Pemasok">
                                ...
                                <form id="delete-form-{{ $pemasok->id }}" action="{{ route('pemasok.destroy', $pemasok->id) }}" method="POST" style="margin: 0;">
                                    @csrf @method('DELETE')
                                    <button type="button" class="btn-action text-white shadow-sm" style="background-color: #ef4444;" title="Hapus Pemasok" onclick="confirmDelete('{{ $pemasok->id }}')">
                ...
                @empty
                    <tr><td colspan="10" class="text-center py-5 text-muted fw-medium"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="mb-2" style="color: #cbd5e1;" viewBox="0 0 16 16"><path d="M.54 3.87.5 3a2 2 0 0 1 2-2h3.672a2 2 0 0 1 1.414.586l.828.828A2 2 0 0 0 9.828 3h3.982a2 2 0 0 1 1.992 2.181l-.637 7A2 2 0 0 1 13.174 14H2.826a2 2 0 0 1-1.991-1.819l-.637-7a1.99 1.99 0 0 1 .342-1.31zM2.19 4a1 1 0 0 0-.996 1.09l.637 7a1 1 0 0 0 .995.91h10.348a1 1 0 0 0 .995-.91l.637-7A1 1 0 0 0 13.81 4H2.19zm4.69-1.707A1 1 0 0 0 6.172 2H2.5a1 1 0 0 0-1 .981l.006.139C1.72 3.042 1.95 3 2.19 3h5.396l-.707-.707z"/></svg><br>Data pemasok belum tersedia.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div class="mobile-card-list">
        @forelse($pemasoks as $pemasok)
            <div class="mobile-card-item">
                <div>
                    <div class="mc-title">{{ $pemasok->nama_pemasok }}</div>
                    <div class="mc-sub">{{ $pemasok->jenis ?? '-' }} &middot; {{ $pemasok->no_telepon ?? '-' }}</div>
                    <div class="mc-sub">PIC: {{ $pemasok->nama_pic ?? '-' }}</div>
                </div>
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end action-dropdown-menu">
                        <li>
                            <a class="dropdown-item" href="{{ route('pemasok.edit', $pemasok->id) }}">
                        ...
                        <li>
                            <button type="button" class="dropdown-item text-danger" onclick="confirmDelete('{{ $pemasok->id }}')">
                ...
                <form id="delete-form-{{ $pemasok->id }}" action="{{ route('pemasok.destroy', $pemasok->id) }}" method="POST" class="d-none">
                    @csrf @method('DELETE')
                </form>
            </div>
        @empty
    ...
    @php
        $total = $pemasoks->total();
        $first = $pemasoks->firstItem() ?? ($total ? 1 : 0);
        $last  = $pemasoks->lastItem() ?? $total;
    @endphp
    ...
    <div class="mt-4 d-flex flex-column flex-md-row justify-content-between align-items-center">
        ...
        <div>
            {{ $pemasoks->appends(request()->query())->links('pagination::bootstrap-5') }}
        </div>
    </div>
</div>
